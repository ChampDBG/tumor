    ...
    for fcount, ImgFile in enumerate(List_filenames):
        ...
        if DF_mini_tumor.shape[0] > 0:
            ...
            for idx_node, idx_row in DF_mini_tumor.iterrows():
                ...
                img[img == -2048] = -1
                np.save(Path_save + 'Imgs_l_%04d_%06d.npy' % (fcount, idx_node), img)
                tmp_ImgName = 'Imgs_l_%04d_%06d.npy' % (fcount, idx_node)
                saveInfo = np.array([tmp_ImgName, v_center[0], v_center[1], v_center[2], diam, 1])
                np.savetxt(Path_save + 'Labs_l_%04d_%06d.csv' % (fcount, idx_node), saveInfo, fmt = '%s', newline = ',')
